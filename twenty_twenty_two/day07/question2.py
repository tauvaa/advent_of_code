...
def find_dir_size(directory, dir_sizes):
    ...
    if len(directory["directories"]) == 0:
        ...
        return directory["file_size"] + sum(
            [
                find_dir_size(dir_sizes[d], dir_sizes)
...
for f in dir_sizes:
    ...
    total_sizes[f] = tocheck

total_availible = 70000000
total_needed = 30000000
total_required = total_needed - (total_availible - total_sizes["/"])

sorted_sizes = list(total_sizes.values())
sorted_sizes.sort(reverse=True)
for i in range(len(sorted_sizes)):
    if sorted_sizes[i] < total_required:
