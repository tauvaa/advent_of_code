...
class FishSchool:
    ...
    def move_day(self):
        if self.current_day in self.spawn_days:
            ...
            if (self.current_day + 9) not in self.spawn_days:
                self.spawn_days[self.current_day + 9] = 0
            self.spawn_days[self.current_day + 9] += num_new_fish

            if (self.current_day + 7) not in self.spawn_days:
                self.spawn_days[self.current_day + 7] = 0
            self.spawn_days[self.current_day + 7] += num_new_fish
        self.current_day += 1
